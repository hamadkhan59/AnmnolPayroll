...
{
    public class BuildFourFeeChallan : BasicPdf
    {
        int fine;
        ...
        public PdfDocument CreatePdf(string className, string secName, IList SrNo, IList rollNo, IList stdName, IList challanId, IList issueChallanId, IList studId, string financeAccountType, string financeAccountId, IList fineList, string currentMonth, int branchId)
        {
            ...
            try
            {
                ...
                fine = (int)feePlanRepo.GetFine(branchId).Fine;
                //IssueChallanConfig iChlnCnfg = feePlanRepo.GetFine(branchId).FinePerDay;
        ...
        private int CreatePdf(string className, string secName, string srNo, string rollNo, string stdName, int challanId, int issueChallanId, int studId, PdfDocument pdfdoc, string financeAccountType, string financeAccountId, string currentMonth, Student student, int studentFine, PdfPage page, XGraphics grph, int height)
        {
            ...
            try
            {
                ...
                for (int part = 0; part < 2; part++)
                {
                    ...
                    grph.DrawString(schoolConfig.SchoolName, font, XBrushes.Black, new XRect(startPoint + part * 260, 20 + height, 230, 10), XStringFormats.TopCenter);

                    grph.DrawLine(XPens.Black, startPoint - 5 + part * 260, 35 + height, startPoint + 240 + part * 260, 35 + height);
                    font = new XFont("Verdana", 10, XFontStyle.Regular);
                    if (part == 0)
                        grph.DrawString("Account Office's Copy", font, XBrushes.Black, new XRect(startPoint + part * 260, 40 + height, 230, 10), XStringFormats.TopCenter);
                    else if (part == 1)
                        grph.DrawString("Parent's Copy", font, XBrushes.Black, new XRect(startPoint + part * 260, 40 + height, 230, 10), XStringFormats.TopCenter);
                    else if (part == 2)
                        grph.DrawString("Bank's Copy", font, XBrushes.Black, new XRect(startPoint + part * 260, 40 + height, 230, 10), XStringFormats.TopCenter);
                    ...
                    for (int k = 0; k < accountList.Count && k < 2; k++)
                    {
                        grph.DrawString(accountList[k].AccountName, font, XBrushes.Black, new XRect(startPoint - 5 + part * 260, 60 + (k * 10) + height, 230, 10), XStringFormats.TopCenter);
                    }
                    ...
                    if (schoolConfig.SchoolLogo != null)
                    {
                        ...
                        if (part == 1)
                        {
                            grph.DrawImage(schImage, startPoint + 5 + part * 255, 40 + height);
                        }
                        else
                        {
                            grph.DrawImage(schImage, startPoint + part * 255, 40 + height);
                        }
                    }
                    ...
                    if (student.StdImage != null)
                    {
                        ...
                        grph.DrawImage(image, startPoint + part * 260 + 195, 40 + height);
                    }

                    height = height - 9;
                    grph.DrawLine(XPens.Black, startPoint - 5 + part * 260, 90 + height, startPoint + 240 + part * 260, 90 + height);
                    font = new XFont("Verdana", 8, XFontStyle.Regular);
                    grph.DrawString("Challan No: " + challanNo, font, XBrushes.Black, new XRect(startPoint + part * 260, 95 + height, 230, 10), XStringFormats.TopCenter);
                    ...
                    grph.DrawString("Fee Of Month : " + month, font, XBrushes.Black, new XRect(startPoint + part * 260, 105 + height, 230, 10), XStringFormats.TopCenter);
                    grph.DrawString("Issue Date : " + issuedDate + "  Due Date: " + dueDate, font, XBrushes.Black, new XRect(startPoint + part * 260, 117 + height, 230, 10), XStringFormats.TopCenter);
                    ...
                    grph.DrawString(srNo + ", " + stdName + ", " + rollNo, font, XBrushes.Black, new XRect(startPoint + part * 260, 130 + height, 230, 10), XStringFormats.TopCenter);
                    font = new XFont("Verdana", 8, XFontStyle.Regular);
                    grph.DrawString("Class : " + className + "  " + "Section : " + secName, font, XBrushes.Black, new XRect(startPoint + part * 260, 140 + height, 230, 10), XStringFormats.TopCenter);

                    grph.DrawLine(XPens.Black, startPoint - 5 + part * 260, 152 + height, startPoint + 240 + part * 260, 152 + height);
                    font = new XFont("Verdana", 8, XFontStyle.Bold);
                    grph.DrawString("Not To Be Deposited After 15th Of Each Month", font, XBrushes.Black, new XRect(startPoint + part * 260, 154 + height, 230, 10), XStringFormats.TopCenter);

                    height = height + 9;
                    grph.DrawRectangle(XPens.Black, startPoint - 5 + part * 260, 156 + height, 160, 15);
                    grph.DrawRectangle(XPens.Black, startPoint + 155 + part * 260, 156 + height, 85, 15);
                    font = new XFont("Verdana", 8, XFontStyle.Bold);
                    grph.DrawString("Fee Head", font, XBrushes.Black, new XRect(startPoint + part * 260, 158 + height, 155, 10), XStringFormats.TopLeft);
                    grph.DrawString("Amount", font, XBrushes.Black, new XRect(startPoint + 160 + part * 260, 158 + height, 80, 10), XStringFormats.TopLeft);
                    ...
                    if (challanDetail != null)
                    {
                        foreach (ChallanDetailViewModel detail in challanDetail)
                        {
                            if (detail.Amount > 0)
                            {
                                grph.DrawRectangle(XPens.Black, startPoint - 5 + part * 260, 158 + i * 13 + height, 160, 13);
                                grph.DrawRectangle(XPens.Black, startPoint + 155 + part * 260, 158 + i * 13 + height, 85, 13);
                                total += Convert.ToInt32(detail.Amount);
                                grph.DrawString(detail.Name, font, XBrushes.Black, new XRect(startPoint + part * 260, 160 + i * 13 + height, 155, 10), XStringFormats.TopLeft);
                                grph.DrawString(detail.Amount.ToString(), font, XBrushes.Black, new XRect(startPoint + 160 + part * 260, 160 + i * 13 + height, 80, 10), XStringFormats.TopLeft);
                                i++;
                            }
                        }
                    }
                    ...
                    if (prevIssueChallan == null)
                    {
                        ...
                        {
                            font = new XFont("Verdana", 8, XFontStyle.Bold);
                            grph.DrawRectangle(XPens.Black, startPoint - 5 + part * 260, 158 + i * 13 + height, 245, 13);
                            //grph.DrawRectangle(XPens.Black, 175 + part * 260, 158 +i * 13 + height, 85, 15);
                            grph.DrawString("Extra Charges", font, XBrushes.Black, new XRect(startPoint + part * 260, 160 + i * 13 + height, 155, 10), XStringFormats.TopLeft);
                            font = new XFont("Verdana", 8, XFontStyle.Regular);
                            ...
                            foreach (StudentExtraChargesDetail detail in extraChargesDetail)
                            {
                                if (detail.HeadAmount > 0)
                                {
                                    ...
                                    grph.DrawString(detail.FeeHead.Name, font, XBrushes.Black, new XRect(startPoint + part * 260, 160 + i * 13 + height, 155, 10), XStringFormats.TopLeft);
                                    grph.DrawString(detail.HeadAmount.ToString(), font, XBrushes.Black, new XRect(startPoint + 160 + part * 260, 160 + i * 13 + height, 80, 10), XStringFormats.TopLeft);
                                    i++;
                                }
                            }
                        }
                        ...
                        if (extraCount > 0)
                        {
                            ...
                            foreach (var detail in CurrentExtras)
                            {
                                ...
                                if (pendingAmount > 0)
                                {
                                    ...
                                    grph.DrawString(detail.FeeHead.Name, font, XBrushes.Black, new XRect(startPoint + part * 260, 160 + i * 13 + height, 155, 10), XStringFormats.TopLeft);
                                    grph.DrawString(pendingAmount.ToString(), font, XBrushes.Black, new XRect(startPoint + 160 + part * 260, 160 + i * 13 + height, 80, 10), XStringFormats.TopLeft);
                                    i++;
                                }
                            }
                        }
                    ...
                    if ((arrearDetail != null && arrearDetail.Count > 0) || (lastMonthFee != null && lastMonthFee.Count > 0))
                    {
                        ...
                        grph.DrawRectangle(XPens.Black, startPoint - 5 + part * 260, 158 + i * 13 + height, 245, 13);
                        //grph.DrawRectangle(XPens.Black, 175 + part * 260, 158 +i * 13 + height, 85, 15);
                        grph.DrawString("Arrears", font, XBrushes.Black, new XRect(startPoint + part * 260, 160 + i * 13 + height, 155, 10), XStringFormats.TopLeft);
                        font = new XFont("Verdana", 8, XFontStyle.Regular);
                            ...
                            if (arrearDetail != null && arrearDetail.Count > 0)
                            {
                                foreach (FeeArrearViewModel detail in arrearDetail)
                                {
                                    if (detail.ArrearAmount > 0)
                                    {
                                        ...
                                        grph.DrawString(detail.HeadName, font, XBrushes.Black, new XRect(startPoint + part * 260, 160 + i * 13 + height, 155, 10), XStringFormats.TopLeft);
                                        grph.DrawString(detail.ArrearAmount.ToString(), font, XBrushes.Black, new XRect(startPoint + 160 + part * 260, 160 + i * 13 + height, 80, 10), XStringFormats.TopLeft);
                                        i++;
                                    }
                                }
                            }
                    ...
                    if (studentAnnualCharges != null && studentAnnualCharges.Count > 0)
                    {
                        ...
                        if (annualCharges != null && annualCharges.Charges > 0)
                        {
                            ...
                            grph.DrawRectangle(XPens.Black, startPoint - 5 + part * 260, 158 + i * 13 + height, 160, 13);
                            grph.DrawRectangle(XPens.Black, startPoint + 155 + part * 260, 158 + i * 13 + height, 85, 13);
                            grph.DrawString("Annual Charges", font, XBrushes.Black, new XRect(startPoint + part * 260, 160 + i * 13 + height, 155, 10), XStringFormats.TopLeft);
                            grph.DrawString(annualCharges.Charges.ToString(), font, XBrushes.Black, new XRect(startPoint + 160 + part * 260, 160 + i * 13 + height, 80, 10), XStringFormats.TopLeft);
                            i++;
                    ...
                    grph.DrawRectangle(XPens.Black, startPoint + 155 + part * 260, 158 + i * 13 + height, 85, 13);
                    total -= advance;
                    grph.DrawString("Advance", font, XBrushes.Black, new XRect(startPoint + part * 260, 160 + i * 13 + height, 155, 10), XStringFormats.TopLeft);
                    grph.DrawString(advance.ToString(), font, XBrushes.Black, new XRect(startPoint + 160 + part * 260, 160 + i * 13 + height, 80, 10), XStringFormats.TopLeft);
                    i++;

                    grph.DrawRectangle(XPens.Black, startPoint - 5 + part * 260, 158 + i * 13 + height, 160, 13);
                    grph.DrawRectangle(XPens.Black, startPoint + 155 + part * 260, 158 + i * 13 + height, 85, 13);
                    grph.DrawString("Fine", font, XBrushes.Black, new XRect(startPoint + part * 260, 160 + i * 13 + height, 155, 10), XStringFormats.TopLeft);
                    grph.DrawString(studentFine.ToString(), font, XBrushes.Black, new XRect(startPoint + 160 + part * 260, 160 + i * 13 + height, 80, 10), XStringFormats.TopLeft);
                    i++;

                    font = new XFont("Verdana", 8, XFontStyle.Bold);
                    grph.DrawRectangle(XPens.Black, startPoint - 5 + part * 260, 158 + i * 13 + height, 160, 13);
                    grph.DrawRectangle(XPens.Black, startPoint + 155 + part * 260, 158 + i * 13 + height, 85, 13);
                    grph.DrawString("Total", font, XBrushes.Black, new XRect(startPoint + part * 260, 160 + i * 13 + height, 155, 10), XStringFormats.TopLeft);
                    total += studentFine;
                    grph.DrawString(total.ToString(), font, XBrushes.Black, new XRect(startPoint + 160 + part * 260, 160 + i * 13 + height, 80, 10), XStringFormats.TopLeft);
                    ...
                    string addressLink = schoolBranch.ADDRESS;
                    if (addressLink != null && addressLink.Length > 5)
                    {
                        XFont camFont = new XFont("Verdana", 9, XFontStyle.Bold);
                        if (addressLink.Length > 30)
                            camFont = new XFont("Verdana", 8, XFontStyle.Bold);
                        else if (addressLink.Length > 40)
                            camFont = new XFont("Verdana", 7, XFontStyle.Bold);
                        else if (addressLink.Length > 50)
                            camFont = new XFont("Verdana", 6, XFontStyle.Bold);

                        grph.DrawString(addressLink, camFont, XBrushes.Black, new XRect(startPoint + 5 + part * 260, 155 + i * 13 + height, 230, 10), XStringFormats.TopLeft);
                    }
                    ...
                    if (campusEmail != null && campusEmail.Length > 5)
                    {
                        ...
                        grph.DrawString(campusEmail, camFont, XBrushes.Black, new XRect(startPoint + 5 + part * 260, 170 + i * 13 + height, 230, 10), XStringFormats.TopLeft);
                    }
                    ...
                    if (campusPhoneNumber != null && campusPhoneNumber.Length > 5)
                    {
                        ...
                        grph.DrawString(campusPhoneNumber, camPFont, XBrushes.Black, new XRect(startPoint + 5 + part * 260, 185 + i * 13 + height, 230, 10), XStringFormats.TopLeft);
                    }

                    grph.DrawLine(XPens.Black, startPoint - 5 + part * 260, 200 + i * 13 + height, startPoint + 240 + part * 260, 200 + i * 13 + height);
                    if (fine > 0)
                    {
                        //grph.DrawString("Note : Fine of Rs." + fine.ToString() + " is charged per day on late fee submission", font, XBrushes.Black, new XRect(startPoint + 5 + part * 260, 203 + i * 13 + height, 230, 10), XStringFormats.TopLeft);
                        grph.DrawString("Note : Fine of Rs." + fine.ToString() + " will be implemented after 15th of every month.", font, XBrushes.Black, new XRect(startPoint + 5 + part * 260, 203 + i * 13 + height, 230, 10), XStringFormats.TopLeft);
                    }
                    grph.DrawRectangle(XPens.Black, startPoint - 5, 15 + height, 245, 200 + i * 13);
                    grph.DrawRectangle(XPens.Black, startPoint + 255, 15 + height, 245, 200 + i * 13);
                    //grph.DrawRectangle(XPens.Black, 535, 15 + height, 245, 160 + i * 13);

                    pageHeight = 160 + (i + 11) * 13;
                    page.Height = (160 + (i + 11) * 13) * 2 + 120;
