...
{
    public class BuildFeeChallan : BasicPdf
    {
        int fine;
        ...
        private void CreatePdf(string className, string secName, string srNo, string rollNo, string stdName, int challanId, int issueChallanId, int studId, PdfDocument pdfdoc, string financeAccountType, string financeAccountId, string currentMonth, Student student, int studentFine)
        {
            try
            {
                ...
                for (int part = 0; part < 3; part++)
                {
                    ...
                    font = new XFont("Verdana", 8, XFontStyle.Regular);
                    grph.DrawString("S.No_______________ Officer CASHIER", font, XBrushes.Black, new XRect(25 + part * 260, 500, 230, 10), XStringFormats.TopLeft);
                    ...
                    grph.DrawString("Rs. 300 will be charge as fine after Due Date of every month.", font, XBrushes.Black, new XRect(25 + part * 260, 520, 230, 10), XStringFormats.TopLeft);
                    grph.DrawString("Rs. 10 will be charge for duplicate voucher.", font, XBrushes.Black, new XRect(25 + part * 260, 530, 230, 10), XStringFormats.TopLeft);
                    grph.DrawString("While depositing fee as per instructions of State Bank Of Pakistan,", font, XBrushes.Black, new XRect(25 + part * 260, 540, 230, 10), XStringFormats.TopLeft);
                    grph.DrawString("the depositer should carry their CNIC copy while depositing fee.", font, XBrushes.Black, new XRect(25 + part * 260, 550, 230, 10), XStringFormats.TopLeft);
                }
                LogWriter.WriteProcEndLog(this.GetType().Name, MethodBase.GetCurrentMethod().Name);
            }
            catch (Exception ex)
