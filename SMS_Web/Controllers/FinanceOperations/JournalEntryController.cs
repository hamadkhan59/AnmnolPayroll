...
namespace SMS_Web.Controllers.FinanceOperations
{
    public class JournalEntryController : Controller
    {
        ...
        public ActionResult CRVoucher()
        {
            ...
            if (UserPermissionController.CheckUserPermission(Session.SessionID, ConstHelper.FA_CASH_RECEIPT_VOUCHER) == false)
            {
                return RedirectToAction("Index", "NoPermission");
            }
            ...
            try
            {
                ...
                ViewData["fourthLvlAccounts"] = SessionHelper.FinanceFourthLvlAccountList(Session.SessionID);
                ViewData["fifthLvlAccounts"] = SessionHelper.FinanceFifthLvlAccountListWitoutReceipts(Session.SessionID);
                ViewBag.FifthLvlAccountId = new SelectList(accountRepo.GetFinanceAccountsFifthLvl("Cash", branchId), "Id", "AccountName");
                ViewData["Error"] = errorCode;
                errorCode = 0;
            }
            catch (Exception ex)
        ...
        public ActionResult CPVoucher()
        {
            ...
            if (UserPermissionController.CheckUserPermission(Session.SessionID, ConstHelper.FA_CASH_PAYMENT_VOUCHER) == false)
            {
                return RedirectToAction("Index", "NoPermission");
            }
        ...
        private void CreateJournalEntry(string ChequeNo, DateTime EntryDate, int EntryAmount, string CreditDescription, string EntryType, int branchId)
        {
            try
            {
                ...
                string description = FinanceHelper.AddEntryDebitDetail(je.EntryId, debitEntryInfo, EntryType, branchId);
        ...
        public ActionResult JournalInquiry()
        {
            ...
            if (UserPermissionController.CheckUserPermission(Session.SessionID, ConstHelper.FA_FINANCE_JOURNAL_INQUIRY) == false)
            {
                return RedirectToAction("Index", "NoPermission");
            }
            ...
            int FinanceThirdLvlAccount = 0, int FinanceFourthLvlAccount = 0, int FinanceFifthLvlAccount = 0, int FinanceMode = 0)
        {
            try
            {
                ...
                int branchId = UserPermissionController.GetLoginBranchId(Session.SessionID);
                var entryList = accountRepo.SearchJournalEntries(FromDate, ToDate, FinanceFirstLvlAccount, FinanceSeccondLvlAccount,
